...
export default class CreateType extends Vue {
    ...
    @Prop({ required: true })
    SaveHandler: (result: IPlatformDirectory, closeModal: boolean) => void;

    @Prop({ required: false })
    SelectedObjectType: IPlatformDirectory | undefined;
    ...
    onSelectedObjectTypeChanged(val: IPlatformDirectory | undefined, oldVal: IPlatformDirectory | undefined): void {
        if (val) {
        ...
        this.$validator.validateAll().then((result: boolean) => {
            if (result) {
                ...
                this.storeImpl(this.model)
                    .then((result: IPlatformDirectory) => {
                        if (closeModal) {
                            this.resetModel();
                            this.$validator.reset();
                        }
