...
import { IRaidDirectory } from "../../../Store/Modules/Dictionaries/types";
...
export default class CreateType extends Vue {
	...
	@Action(Actions.STORE_RAID, {namespace: namespace })
    storeImpl: (model: IRaidDirectory) => Promise<IRaidDirectory>;

    @Prop({ required: true })
    SaveHandler: (result: IRaidDirectory, closeModal: boolean) => void;

    @Prop({ required: false })
    SelectedObjectType: IRaidDirectory | undefined;
    ...
    onSelectedObjectTypeChanged(val: IRaidDirectory | undefined, oldVal: IRaidDirectory | undefined): void {
        if (val) {
