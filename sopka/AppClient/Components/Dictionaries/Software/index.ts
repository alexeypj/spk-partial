...
import { ISoftwareDirectory, IDirectoryState, ISoftwareFilter } from "../../../Store/Modules/Dictionaries/types";
...
export default class ObjectTypes extends Vue {
    ...
    State: IDirectoryState<ISoftwareDirectory, ISoftwareFilter>;

    @Action(Actions.FETCH_SOFTWARE, { namespace: namespace })
    Fetch: (filter: ISoftwareFilter) => Promise<void>;
    ...
    SetFilter: (filter: ISoftwareFilter) => void;


    showCreateModal: boolean = false;
    showFilterModal: boolean = false;
    selectedObjectTypeIndex: number | null = null;
    modalTitle: string = "Добавить ПО";
    showError: boolean = false;
    errorText: string = "";

    //фильтр для окна фильтров
    additionalFilter: ISoftwareFilter;
    ...
    applyFilter(filter: ISoftwareFilter) {
        this.SetFilter(filter);
        this.Fetch(filter);
    }

    get selectedObjectType(): ISoftwareDirectory | null {
    ...
    removeDic(dic: ISoftwareDirectory): void {
    ...
    onDicSaved(dic: ISoftwareDirectory, closeModal: boolean) {
        this.Fetch(this.State.Filter);
        if (closeModal) {
            this.closeModal();
        }
    ...
    applyAdditionalFilter(filter: ISoftwareFilter) {
    ...
    public selectorChange(value: any, dic: ISoftwareDirectory): void {
        switch (value) {
