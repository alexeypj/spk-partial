...
import { IObjectTypeDirectory } from "../../../Store/Modules/Dictionaries/types";
...
export default class CreateType extends Vue {
	...
	@Action(Actions.STORE_OBJECT_TYPE, {namespace: namespace })
    storeImpl: (model: IObjectTypeDirectory) => Promise<IObjectTypeDirectory>;

    @Prop({ required: true })
    SaveHandler: (result: IObjectTypeDirectory, closeModal: boolean) => void;

    @Prop({ required: false })
    SelectedObjectType: IObjectTypeDirectory | undefined;
    ...
    private model: IObjectTypeDirectory = {
        Id: 0,
        Title: "",
        Description: ""
    };
    ...
    onSelectedObjectTypeChanged(val: IObjectTypeDirectory | undefined, oldVal: IObjectTypeDirectory | undefined): void {
        if (val) {
    ...
    get Model(): IObjectTypeDirectory {
        return this.model;
    }
        ...
        this.$validator.validateAll().then((result: boolean) => {
            if (result) {
                ...
                this.storeImpl(this.model)
                    .then((result: IObjectTypeDirectory) => {
                        if (closeModal) {
                            this.resetModel();
                            this.$validator.reset();
                        }
